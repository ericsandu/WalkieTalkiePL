...
\begin{figure}[H]
    ...
    \tikzstyle{decision} = [diamond, draw, aspect=2.5, inner sep=0pt, minimum width=1.6cm, minimum height=1.2cm, align=center, fill=yellow!15, font=\scriptsize]
...
\begin{description}
    ...
    \begin{itemize}
        ...
        \item \textbf{S2 (SCAN) și S3 (RETRY):} Dispozitivul scanează frecvențele căutând un semnal; dacă nu este găsit, trece prin \texttt{RETRY} care scanează cu o putere de transmitere crescătoare până găsește un dispozitiv.
        \item \textbf{S4 (SHAKE):} Odată detectat un semnal, se așteaptă confirmarea handshake-ului  (\texttt{HS\_OK}).
        \item \textbf{S7 (ERR):} Stare de eroare. Se activează dacă handshake-ul eșuează, generând semnalul \texttt{ERR\_LED}.
    ...
    \begin{itemize}
        ...
        \item \textbf{S11 (LSTN):} Stare activă de recepție. Când \texttt{RX\_SIG} este detectat, automatul activează ieșirile \texttt{SPK\_EN} și \texttt{RX\_LED}.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{S13 (TRSM):} Stare activă de transmisie. Menține active ieșirile \texttt{MIC\_EN} și \texttt{TX\_LED} atâta timp cât \texttt{PTT} este apăsat.
