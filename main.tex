\documentclass[a4paper,12pt]{article}
\usepackage[justification=centering]{caption}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[romanian]{babel}
\usepackage[table]{xcolor}
\usepackage{array}
\usepackage{graphicx}
\usepackage{geometry}
\usepackage{diagbox}
\usepackage{amsmath}
\usepackage{float}
\usepackage{tikz}
\usetikzlibrary{shapes, arrows, arrows.meta, positioning, circuits.logic.US, calc, fit, backgrounds}

% Setări pagină
\geometry{margin=2.5cm}

\begin{document}

% 1. Pagina de titlu
\input{titlepage.tex}

\newpage
\tableofcontents
\newpage

% 2. Descrierea proiectului
\section{Descrierea funcționării automatului}
\input{description.tex}

% 3. Schema bloc
\section{Schema bloc a automatului}
\input{block_diagram.tex}
\newpage

% 4. Organigrama
\section{Organigrama automatului}
\input{organigram.tex}
\newpage

% % 5. Ecuațiile de stare
\section{Identificarea ecuațiilor de stare}
\input{state_equations.tex}
\newpage

% 6. Implementare CBB + MUX
\section{Implementarea cu CBB JK și Multiplexoare 4:1}
\input{implementation_cbb.tex}
\newpage

% 7. Ieșiri
\section{Implementarea ieșirilor cu porți logice}
\input{outputs.tex}
\newpage

% 8. Organigramă cu microinstrucțiuni
\section{Implementarea cu microinstrucțiuni variabile}
\subsection{Organigrama pentru implementarea cu microinstrucțiuni}
\input{organigram_micro.tex}

% 9. Calculul lungimii microinstrucțiunii
\subsection{Calculul lungimii microinstrucțiunii}
\input{lenght_micro.tex}

% 10. Proiectarea unității de comandă microprogramată
\subsection{Proiectarea unității de comandă microprogramată}
\input{control_unit.tex}

% 11. Alegerea componentelor digitale
\subsection{Alegerea componentelor digitale}
\input{components.tex}

% 12. Completarea memoriei de microprogram
\subsection{Completarea memoriei de microprogram}
\input{memory.tex}
\end{document}
