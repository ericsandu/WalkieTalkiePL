...
\subsubsection*{\textbf{Selectori:} $Q_1, Q_0$}

% --- Q3 ---
\subsection{Implementarea etajului $Q_3$}
Logica intrărilor \textbf{MUX} depinde de $Q_2$.

\begin{figure}[H]
    \centering
    % Resizebox forces the image to fit the text width, scaling everything up
    ...
    \begin{tikzpicture}[font=\footnotesize]
        ...
        % Tabel K (Dreapta - Spațiat mai larg pentru a umple pagina)
        \node (tableK) at (8.0,0) [anchor=north west] {
            \renewcommand{\arraystretch}{\tablestretch}
            \tablefontsize
            \begin{tabular}{|c|c|c|}
                \hline
        ...
        % --- SCHEMA LOGICĂ ---
        % Adjusted Y coordinates to stretch vertical space
        \def\topY{-7.5}     
        \def\botY{-12.5}    
        \def\midY{-10.0}    
        \def\muxX{3.5}
...
Logica intrărilor \textbf{MUX} depinde de $Q_3$.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[font=\footnotesize]
        ...
        % Parametri Expansion
        \def\topY{-7.5} \def\botY{-12.5} \def\midY{-10.0}
        \def\muxX{3.5} \def\cbbX{11.0} \def\pinOffset{0.6} \def\muxPinSpacing{0.65}
        ...
        \foreach \i/\lbl/\val in {
            ...
            \draw (muxJ.west) 
            ++(0, \i*\muxPinSpacing) -- node[midway, above, font=\tiny, inner sep=1pt] {\lbl} ++(-0.8, 0) coordinate(startline);
            \node[left, align=right] at (startline) {\small \val};
        }
        \foreach \i/\lbl/\val in {
            1.5/00/$0$,
...
Logica intrărilor \textbf{MUX} depinde de $Q_3$ și $Q_2$.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[font=\footnotesize]
        ...
        % Tabel K (Dreapta)
        % Moved further right (x=9.0) to accommodate wider tables + scaling
        \node (tableK) at (9.0,0) [anchor=north west] {
            \renewcommand{\arraystretch}{\tablestretch}
            \tablefontsize
            \setlength{\tabcolsep}{3pt}
            \begin{tabular}{|c|c|c|c|c|}
        ...
        % Parametri Expanded for Large Page Fit
        \def\topY{-7.5} \def\botY{-12.5} \def\midY{-10.0}
        \def\muxX{4.0} \def\cbbX{12.5} \def\pinOffset{0.6} \def\muxPinSpacing{0.65}
        ...
        % Intrări
        \foreach \i/\lbl/\val in {
          1.5/00/$\overline{Q_2}Q_3 + \overline{Q_2} \cdot \bar{P}\bar{C} + Q_2\overline{Q_3}\cdot \bar{H}$,
          0.5/01/$\overline{Q_3} + \overline{Q_2} + \bar{T}$,
            -0.5/10/$*$, 
            -1.5/11/$*$
        } {
            \draw (muxJ.west) ++(0, 
            \i*\muxPinSpacing) -- node[midway, above, font=\tiny, inner sep=1pt] {\lbl} ++(-0.8, 0) coordinate(startline);
            \node[left, align=right] at (startline) {\small \val};
        }
        \foreach \i/\lbl/\val in {
            1.5/00/$*$,
            0.5/01/$*$, 
            -0.5/10/$Q_2\overline{Q_3} + \overline{Q_3} \cdot R + \overline{Q_2}Q_3 \cdot \bar{R}$, 
            -1.5/11/$Q_2 + \overline{Q_3} \cdot R$
        } {
            \draw (muxK.west) ++(0, \i*\muxPinSpacing) -- node[midway, above, font=\tiny, inner sep=1pt] {\lbl} ++(-0.8, 0) coordinate(startline);
            \node[left, align=right] at (startline) {\small \val};
        }
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[font=\footnotesize]
        ...
        % Intrări
        \foreach \i/\lbl/\val in {
          1.5/00/$Q_2 + \overline{Q_3} \cdot P$, 
            0.5/01/$*$, 
            -0.5/10/$Q_2Q_3 + Q_3 \cdot R + \overline{Q_2}\overline{Q_3} \cdot \bar{R}$, 
            -1.5/11/$*$
        } {
            \draw (muxJ.west) 
            ++(0, \i*\muxPinSpacing) -- node[midway, above, font=\tiny, inner sep=1pt] {\lbl} ++(-0.8, 0) coordinate(startline);
