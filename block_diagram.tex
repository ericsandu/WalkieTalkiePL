...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[auto, node distance=1.5cm, >=latex', thick]
        ...
        % --- INTRĂRI ---
        \coordinate (in_ptt) at ($(asm.north west)!0.20!(asm.south west)$);
        \coordinate (in_pair) at ($(asm.north west)!0.35!(asm.south west)$);
        \coordinate (in_ch) at ($(asm.north west)!0.50!(asm.south west)$);
        \coordinate (in_rx) at ($(asm.north west)!0.65!(asm.south west)$);
        \coordinate (in_hs) at ($(asm.north west)!0.80!(asm.south west)$);
        ...
        % --- IEȘIRI ---
        % Ieșiri care revin la utilizator
        \coordinate (out_tx) at ($(asm.north east)!0.20!(asm.south east)$);
        \coordinate (out_rx) at ($(asm.north east)!0.35!(asm.south east)$);
        \coordinate (out_err) at ($(asm.north east)!0.50!(asm.south east)$);
        % Ieșiri care merg mai departe la dispizitiv
        \coordinate (out_mic) at ($(asm.north east)!0.65!(asm.south east)$);
        \coordinate (out_spk) at ($(asm.north east)!0.80!(asm.south east)$);
